(function () {
  ...
  function renderAssetPreview(card, path) {
    ...
    fetch(url, { method: 'HEAD', cache: 'no-store' }).then(response => {
      ...
      if (type.startsWith('video/')) {
        ...
        video.src = url; video.muted = true; video.loop = true; video.autoplay = true; video.playsInline = true; video.controls = true;
        preview.appendChild(video);
        video.play().catch(() => {});
      } else if (type.startsWith('image/')) {
        const img = new Image(); img.src = url; img.alt = card.querySelector('.slot-name')?.textContent || 'Mothership image'; preview.appendChild(img);
      } else {
        preview.innerHTML = '<span>No Cloudflare asset found.</span>';
      }
    }).catch(() => { preview.innerHTML = '<span>No Cloudflare asset found.</span>'; });
  }

  function refreshAllAssetPreviews() {
  ...
  document.addEventListener('change', async event => {
    ...
    input.disabled = true; if (deleteBtn) deleteBtn.disabled = true;
    try {
      ...
      input.disabled = false; if (deleteBtn) deleteBtn.disabled = false; input.value = '';
    }
  }, true);
  ...
  document.addEventListener('click', async event => {
    ...
    const status = card.querySelector('.status');
    button.disabled = true; setStatus(status, 'Removing from Cloudflare…');
    try {
  ...
  document.addEventListener('click', async event => {
    ...
    button.disabled = true; setStatus(status, 'Saving to Cloudflare…');
    try {
      const colors = {};
  ...
  function wirePopupControl() {
    ...
    if (!panel || !button || button.dataset.cfWired === '1') return;
    ...
    fetch(assetUrl(cfg.popupPath || 'system/orbit-popup.json'), { cache: 'no-store' }).then(r => r.ok ? r.json() : { enabled: false }).then(data => {
      enabled = data?.enabled === true; render(); setStatus(status, enabled ? 'Orbit popup is ON.' : 'Orbit popup is OFF for public visitors.', 'ok');
    }).catch(() => { enabled = false; render(); setStatus(status, 'Could not read popup status; keeping it OFF.', 'error'); });
    button.addEventListener('click', async event => {
      event.preventDefault(); event.stopImmediatePropagation();
      const next = !enabled;
      button.disabled = true; setStatus(status, next ? 'Turning Orbit popup on…' : 'Turning Orbit popup off…');
      try {
        ...
        enabled = next; render(); setStatus(status, enabled ? 'Orbit popup is ON.' : 'Orbit popup is OFF.', 'ok');
      } catch (error) { button.disabled = false; setStatus(status, error.message || 'Could not change popup status.', 'error'); }
    }, true);
  }

  const observer = new MutationObserver(() => { wirePopupControl(); refreshAllAssetPreviews(); });
  observer.observe(document.documentElement, { childList: true, subtree: true });
  window.addEventListener('load', () => { wirePopupControl(); refreshAllAssetPreviews(); });
})();
